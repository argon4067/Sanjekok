...
NUM_ROWS = 1000   # 1페이지당 요청 건수 (API가 1000까지 응답)
MAX_PAGE = 7      # 최대 7페이지(=7000건)까지 시도


def crawl_hospitals(max_page: int = MAX_PAGE):
    """
    SAFEMAP 산재지정병원(IF_0025) 데이터를 페이지 단위로 전부 가져와서
    ...
    try:
        for page in range(1, max_page + 1):
            ...
            # 이 페이지가 꽉 차지 않았으면 마지막 페이지라고 판단
            if len(hospitals) < NUM_ROWS:
                print("✅ 마지막 페이지(행 수 < NUM_ROWS)로 판단, 종료")
                break

        if page == max_page and len(hospitals) == NUM_ROWS:
